...
def asegurar_recamara(juego):
    """Recarga la escopeta si quedo vacia y anuncia la nueva ronda.
    ...
    if juego.escopeta.esta_vacia():
        reales, fogueo = juego.iniciar_ronda()
...
def turno_de_disparo(juego):
    ...
    if bala == TipoBala.REAL:
        ...
        if tenia_segueta:
            console.print("[blue]*click* Cartucho de FOGUEO. La Segueta se gasto sin efecto.[/blue]")
        else:
            console.print("[bold blue]*click* Cartucho de FOGUEO.[/bold blue]")
...
def jugar():
    ...
    while not juego.esta_terminado():
        ...
        if juego.hay_ganador() is not None:
            ...
            console.print(f"\n[bold green]{ganador_asalto.nombre} gana el asalto![/bold green]")
