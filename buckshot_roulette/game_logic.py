...
class Juego:
    ...
    # ---- Rondas e items ---------------------------------------------
    def items_para_ronda(self, numero_ronda):
        """Progresion de items por ronda DENTRO de un asalto.

        Ronda 1 -> 0 items (sin items), ronda 2 -> 2, ronda 3 -> 3,
        ronda 4 en adelante -> 4. El contador se reinicia en cada asalto nuevo.
        """
        if numero_ronda <= 1:
            return 0
        if numero_ronda == 2:
    ...
    def iniciar_ronda(self, repartir=True):
        ...
        La cantidad de items sigue la progresion items_para_ronda() segun el
        numero de ronda del asalto actual.

        Devuelve la tupla (num_reales, num_fogueo, entregados), donde
        'entregados' es una lista [items_jugador0, items_jugador1] con los
        items realmente recibidos por cada jugador (puede ser lista vacia).
        """
        reales, fogueo = self.escopeta.cargar_aleatoria()
        self.ronda += 1
        entregados = [[], []]
        if repartir:
            cantidad = self.items_para_ronda(self.ronda)
            entregados = self.repartir_items(cantidad)
        return (reales, fogueo, entregados)

    def repartir_items(self, cantidad=None, minimo=1, maximo=2):
        """RF-15: reparte items aleatorios a cada jugador.

        Si 'cantidad' es None se usa el comportamiento clasico (un numero
        aleatorio entre 'minimo' y 'maximo' por jugador). Si se pasa
        'cantidad', se reparte exactamente esa cantidad a cada jugador.

        Los items se eligen al azar del pool ITEMS_DISPONIBLES y pueden
        repetirse. Si el inventario llega al tope (INVENTARIO_MAXIMO), los
        items sobrantes se descartan.

        Devuelve una lista [items_jugador0, items_jugador1] con los items
        realmente entregados a cada jugador.
        """
        entregados = []
        for jugador in self.jugadores:
            n = cantidad if cantidad is not None else random.randint(minimo, maximo)
            recibidos = []
            for _ in range(n):
                item = random.choice(self.ITEMS_DISPONIBLES)
                if jugador.agregar_item(item):
                    recibidos.append(item)
            entregados.append(recibidos)
        return entregados
    ...
    def finalizar_asalto(self):
        ...
        if self.puntaje[indice] >= self.asaltos_para_ganar:
            ...
            # Reinicio completo para el siguiente asalto ("nuevo juego"):
            # HP al maximo, inventarios vacios, escopeta vacia (se recargara),
            # turno al primer jugador y progresion de rondas desde cero.
            for jugador in self.jugadores:
