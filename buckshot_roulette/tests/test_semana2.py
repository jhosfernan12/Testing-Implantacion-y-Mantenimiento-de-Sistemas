...
# ----------------------------------------------------------------------
# PROGRESION DE ITEMS POR RONDA (0, 2, 3, 4...) Y REINICIO POR ASALTO
# ----------------------------------------------------------------------
class TestProgresionItems(unittest.TestCase):

    def test_tabla_de_progresion(self):
        """Ronda 1 -> 0, ronda 2 -> 2, ronda 3 -> 3, ronda 4+ -> 4 items."""
        juego = Juego("Ana", "Beto")
        self.assertEqual(juego.items_para_ronda(1), 0)
        self.assertEqual(juego.items_para_ronda(2), 2)
        self.assertEqual(juego.items_para_ronda(3), 3)
        self.assertEqual(juego.items_para_ronda(4), 4)
        self.assertEqual(juego.items_para_ronda(7), 4)

    @patch("game_logic.random.shuffle", _no_shuffle)
    @patch("game_logic.random.choice", return_value=TipoItem.LUPA)
    def test_primera_ronda_sin_items(self, mock_choice):
        """La primera ronda del asalto no reparte items."""
        juego = Juego("Ana", "Beto")
        juego.iniciar_ronda()  # ronda 1
        self.assertEqual(len(juego.jugadores[0].inventario), 0)
        self.assertEqual(len(juego.jugadores[1].inventario), 0)

    @patch("game_logic.random.shuffle", _no_shuffle)
    @patch("game_logic.random.choice", return_value=TipoItem.LUPA)
    def test_segunda_ronda_da_2_items(self, mock_choice):
        """La segunda ronda reparte 2 items a cada jugador."""
        juego = Juego("Ana", "Beto")
        juego.iniciar_ronda()  # ronda 1 -> 0
        juego.escopeta.recamara = []
        juego.iniciar_ronda()  # ronda 2 -> 2
        self.assertEqual(len(juego.jugadores[0].inventario), 2)
        self.assertEqual(len(juego.jugadores[1].inventario), 2)

    @patch("game_logic.random.shuffle", _no_shuffle)
    @patch("game_logic.random.choice", return_value=TipoItem.LUPA)
    def test_cuarta_ronda_da_4_items(self, mock_choice):
        """De la cuarta ronda en adelante se reparten 4 items (tope)."""
        juego = Juego("Ana", "Beto")
        for _ in range(4):  # rondas 1,2,3,4
            juego.escopeta.recamara = []
            juego.iniciar_ronda()
        # En ronda 4 el inventario llega al tope de 4.
        self.assertEqual(len(juego.jugadores[0].inventario), 4)

    @patch("game_logic.random.shuffle", _no_shuffle)
    @patch("game_logic.random.choice", return_value=TipoItem.LUPA)
    def test_reparto_descarta_sobrantes_por_tope(self, mock_choice):
        """Repartir mas de 4 items descarta los sobrantes (tope de inventario)."""
        juego = Juego("Ana", "Beto")
        entregados = juego.repartir_items(cantidad=6)  # se piden 6
        self.assertEqual(len(juego.jugadores[0].inventario), 4)   # solo entran 4
        self.assertEqual(len(entregados[0]), 4)                   # se reportan 4

    @patch("game_logic.random.shuffle", _no_shuffle)
    def test_progresion_se_reinicia_tras_asalto(self, ):
        """Tras ganar un asalto, el contador de rondas vuelve a 0."""
        juego = Juego("Ana", "Beto", hp=1, asaltos_para_ganar=2)
        juego.escopeta.cargar(num_reales=1, num_fogueo=0)
        juego.ronda = 3  # simulamos que ibamos avanzados en el asalto
        juego.disparar("oponente")  # Beto a 0
        juego.finalizar_asalto()    # nuevo asalto
        self.assertEqual(juego.ronda, 0)
        self.assertTrue(juego.escopeta.esta_vacia())
